...
- **Sempre responda em português (pt-BR).** Todas as explicações, comentários e comunicações devem ser em português. Termos técnicos e identificadores de código permanecem no idioma original.

## Progresso de Tarefas (Obrigatório)

- **Sempre use TaskCreate/TaskUpdate** ao executar qualquer tarefa não-trivial. Antes de começar, crie a lista de tasks com `TaskCreate` e exiba o progresso no terminal.
- Marque cada task como `in_progress` ao iniciar e `completed` ao finalizar.
- O usuário deve sempre visualizar no terminal: quais tasks existem, qual está em andamento e quais já foram concluídas.
